...
# activity checkout (the trigger is the merge closeout, NOT a git hook:
...
# bitty-terminal/activity-workflow.
#
# What it does:
#   1. Exports `carryctx export --pack-format dir` from this repo into a
#      staging clone of bitty-terminal/activity-workflow as
#      <UTC-date>-<activity-main-sha>/.
...
#                  https://github.com/bitty-terminal/activity-workflow.git).
...
SOURCE_REPO="${WORKFLOW_SOURCE_REPO:-activity}"
