...
This document describes how security reports for the `activity` repository
...
[GitHub Security Advisory](https://github.com/bitty-terminal/activity/security/advisories/new).
...
The plugin scaffold requests only `terminal.semantic-read` and
`platform.notify` and contains no credentials, no install-time execution, and
no ambient OS authority. Privacy-relevant behavior is described in
